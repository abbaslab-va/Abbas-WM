...
currentTrial = 0;
    while currentTrial <TotalTrials
        ...
        if currentTrial ~= 1
            priorOutcome = BpodSystem.Data.SessionPerformance(currentTrial - 1);
            if [[priorOutcome== 0]|[priorOutcome== 2]] & ismember(TrialTypes(currentTrial-1),TrialTypes_to_repeat)
                ...
                TrialTypes = [TrialTypes(1:currentTrial-1), New_trial_type, TrialTypes(currentTrial:end)];
                RepeatTrial(currentTrial) = 1;
                
            else
                RepeatTrial(currentTrial) = 0;
...
%     
    ...
    if ~isempty(fieldnames(RawEvents)) % If trial data was returned
        ...
        UpdateTrialTypeOutcomePlot(TrialTypes, BpodSystem.Data, RepeatTrial);
...
function UpdateTrialTypeOutcomePlot(TrialTypes, Data, RepeatTrial)
global BpodSystem
Outcomes = zeros(1,Data.nTrials);
for x = 1:Data.nTrials
    
    if ~isnan(Data.RawEvents.Trial{x}.States.Punish(1)) && RepeatTrial(x)==1
        Outcomes(x) = 2;
    elseif ~isnan(Data.RawEvents.Trial{x}.States.ChoiceOn(1)) && RepeatTrial(x)==1
        Outcomes(x) = 3;
...
Outcomes_for_plot = Outcomes; 
Outcomes_for_plot(find(Outcomes_for_plot==2)) = 0; %failed repeats plotted as failed trials
Outcomes_for_plot(find(Outcomes_for_plot==3)) = 2; %correct repeats plotted as open green circles

TrialTypeOutcomePlot(BpodSystem.GUIHandles.TrialTypeOutcomePlot,'update',Data.nTrials+1,TrialTypes,Outcomes_for_plot);
