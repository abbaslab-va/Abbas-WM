function DMTS_Tri_Sample_stim_only
...
for fill = 1:4 %(outer blocks randomized here) As in 4 true trial repeats of each TrialType + StimType combination
...
TrialTypes = [TrialTypes(end-4:end), TrialTypes]; % Using the last 5 trialtypes as the first 5 for the no stim trial buffer at start of task
...
% Now we select 'Day_X_param_set.mat' instead of individually selecting all
% 4 param files
disp('Select mat file for set of stim parameters to load:')
list_of_param_filenames = uigetfile();
load(list_of_param_filenames);
paramMat = cell(1,4);
paramMat{1} = load(param_set{1});
paramMat{2} = load(param_set{2});
paramMat{3} = load(param_set{3});
paramMat{4} = load(param_set{4});



use_softcode = 1; 
BpodSystem.Data.Laser_params_1 = param_set{1};
BpodSystem.Data.Laser_params_2 = param_set{2};
BpodSystem.Data.Laser_params_3 = param_set{3};
BpodSystem.Data.Laser_params_4 = param_set{4};
...
BpodSystem.Data.Task_name = coder.mfunctionname;
...
BpodSystem.SoftCodeHandlerFunction = 'DMTS_Tri_Sample_stim_only_SoftCode';
...
%     end
    ...
    switch current_stim
        ...
        case 2 % Sample Stim 2
            ...
            ProgramPulsePal(currParams);
        case 3 % Sample Stim 3
            sampleTimerTrig = {'GlobalTimerTrig', 2};
            sampleStim = {'BNC1', 1, 'SoftCode', 1};
            sampleStop = {'SoftCode', 2};
            delayStim = {};
            delayStop = {};
            currParams = paramMat{3}.ParameterMatrix;
            ProgramPulsePal(currParams);
        case 4 % Sample Stim 4
            sampleTimerTrig = {'GlobalTimerTrig', 2};
            sampleStim = {'BNC1', 1, 'SoftCode', 1};
            sampleStop = {'SoftCode', 2};
            delayStim = {};
            delayStop = {};
            currParams = paramMat{4}.ParameterMatrix;
            ProgramPulsePal(currParams);
        % case 4 % Delay Stim 2
        %     sampleTimerTrig = {};
        %     sampleStim = {};
    ...
    sma = AddState(sma, 'Name', 'ITI', 'Timer', S.GUI.ITI/2,...
        'StateChangeConditions', {'Tup', 'ITI2', Manual_Skip, 'SkipTrial'},...
    ...
    sma = AddState(sma, 'Name', 'ITI2', 'Timer', S.GUI.ITI/2,...
        ...
        'Port2In', 'ScanPunish', 'Port3In', 'ScanPunish', Manual_Skip, 'SkipTrial'},...
        'OutputActions', {});
    
    sma = AddState(sma, 'Name', 'ScanPunish', 'Timer', 0,...
        'StateChangeConditions', {'Port1Out', 'ITI2', 'Port2Out', 'ITI2', 'Port3Out', 'ITI2', Manual_Skip, 'SkipTrial'},...
        'OutputActions', {'Valve8', 1});

    sma = AddState(sma, 'Name', 'SampleStimTimer', 'Timer', 0,...
        'StateChangeConditions', {'Tup', 'WaitForSamplePoke', Manual_Skip, 'SkipTrial'},...
        'OutputActions', sampleTimerTrig);
 
     sma = AddState(sma, 'Name', 'SampleStimTimerEW', 'Timer', 0,...
        'StateChangeConditions', {'Tup', 'WaitForSamplePokeEW', Manual_Skip, 'SkipTrial'},...
        'OutputActions', sampleTimerTrig);   
    ...
    sma = AddState(sma, 'Name', 'WaitForSamplePoke', 'Timer', 0,...
        ...
        WrongPortsInSample(2), 'SampleOnHoldPunish', Manual_Skip, 'SkipTrial'],...
    ...
    sma = AddState(sma, 'Name', 'SampleOnHold', 'Timer', .05,...
        'StateChangeConditions', ['Tup', 'SampleOn', 'GlobalTimer2_End', 'SampleStimTimeout', WhichSampleOut, 'WaitForSamplePoke', Manual_Skip, 'SkipTrial'],...
    ...
    sma = AddState(sma, 'Name', 'SampleOnHoldPunish', 'Timer', .05,...
        ...
        WrongPortsOutSample(2), 'WaitForSamplePoke', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [SampleLight, sampleStim]);

    % Probably end the stim here     
    sma = AddState(sma, 'Name', 'SampleOn', 'Timer', SampleValveTime,...
        'StateChangeConditions', {'Tup', 'WaitForDelayPoke', 'GlobalTimer2_End', 'SampleStimTimeout', Manual_Skip, 'SkipTrial'},...
    ...
    sma = AddState(sma, 'Name', 'WaitForSamplePokeEW', 'Timer', 0,...
        ...
        WrongPortsInSample(2), 'SampleOnHoldPunishEW', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [SampleLight, sampleStim]);
    
    sma = AddState(sma, 'Name', 'SampleOnHoldEW', 'Timer', 0.05,...
        'StateChangeConditions', ['Tup', 'SampleOnEW', 'GlobalTimer2_End', 'SampleStimTimeout', WhichSampleOut, 'WaitForSamplePokeEW', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [SampleLight, sampleStim]);
    ...
    sma = AddState(sma, 'Name', 'SampleOnHoldPunishEW', 'Timer', 0.05,...
        ...
        WrongPortsOutSample(2), 'WaitForSamplePokeEW', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [SampleLight, sampleStim]);
    ...
    sma = AddState(sma, 'Name', 'WaitForDelayPoke', 'Timer', 0,...
        'StateChangeConditions', [ WhichDelayIn, 'DelayTimer', 'GlobalTimer2_End', 'SampleStimTimeout', WrongPortsInDelay(1), 'BadDelayPoke', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [DelayLight, sampleStim]);
    
    % Trigger sample off
    sma = AddState(sma, 'Name', 'DelayTimer', 'Timer', 0,...
        'StateChangeConditions', {'Tup', 'DelayOnHold', Manual_Skip, 'SkipTrial'},...
    ...
    sma = AddState(sma, 'Name', 'DelayOnHold', 'Timer', S.GUI.DelayHoldTime,...
        'StateChangeConditions', ['Tup', 'DelayOn', 'GlobalTimer1_End', 'DelayOn', WhichDelayOut, 'DelayWaitForReentry', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [DelayLight, delayStim]);
    
    sma = AddState(sma, 'Name', 'DelayWaitForReentry', 'Timer', 1,...
        'StateChangeConditions', ['Tup', 'EarlyWithdrawal', 'GlobalTimer1_End', 'DelayOn', WhichDelayIn, 'DelayOnHold', Manual_Skip, 'SkipTrial'],...
        'OutputActions', [DelayLight, delayStim]);
    
    sma = AddState(sma, 'Name', 'DelayOn', 'Timer', DelayValveTime,...
        'StateChangeConditions', {'Tup', 'WaitForChoicePoke', Manual_Skip, 'SkipTrial'},...
        'OutputActions', [DelayLight, DelayValve, delayStop, ts.delayReward]);
    
    sma = AddState(sma, 'Name', 'WaitForChoicePoke', 'Timer', 0,...
        'StateChangeConditions', [WhichSampleIn, 'ChoiceOnHold', WrongPortsInDelay(1), 'ChoiceOnHoldPunish', Manual_Skip, 'SkipTrial'],...
        'OutputActions', {});
    
    sma = AddState(sma, 'Name', 'ChoiceOnHold', 'Timer', .05,...
        'StateChangeConditions', ['Tup', 'ChoiceOn', WhichSampleOut, 'WaitForChoicePoke', Manual_Skip, 'SkipTrial'],...
        'OutputActions', {});
    
    sma = AddState(sma, 'Name', 'ChoiceOnHoldPunish', 'Timer', .05,...
        'StateChangeConditions', ['Tup', 'Punish', WrongPortsOutDelay(1), 'WaitForChoicePoke', Manual_Skip, 'SkipTrial'],...
        'OutputActions', {});
    
    sma = AddState(sma, 'Name', 'ChoiceOn', 'Timer', ChoiceValveTime,...
        'StateChangeConditions', {'Tup', 'exit', Manual_Skip, 'SkipTrial'},...
    ...
    sma = AddState(sma, 'Name', 'SamplePunish', 'Timer', 0,...
        ...
        WrongPortsOutSample(2), 'WaitForSamplePoke', 'GlobalTimer2_End', 'SampleStimTimeout', Manual_Skip, 'SkipTrial'},...
    ...
    sma = AddState(sma, 'Name', 'SamplePunishEW', 'Timer', 0,...
        ...
        WrongPortsOutSample(2), 'WaitForSamplePokeEW', 'GlobalTimer2_End', 'SampleStimTimeout', Manual_Skip, 'SkipTrial'},...
        'OutputActions', ['Valve8', 1, sampleStim]);
    ...
    sma = AddState(sma, 'Name', 'EarlyWithdrawal', 'Timer', 3,...
        'StateChangeConditions', {'Tup', 'EarlyWithdrawalTimeout', Manual_Skip, 'SkipTrial'},...
        'OutputActions', ['Valve8', 1, delayStop]);
    
    
    sma = AddState(sma, 'Name', 'BadDelayPoke', 'Timer', 3,...
        'StateChangeConditions', {'Tup', 'WaitForSamplePokeEW', 'GlobalTimer2_End', 'SampleStimTimeout', Manual_Skip, 'SkipTrial'},...
        'OutputActions', ['Valve8', 1, sampleStim]);
    
    sma = AddState(sma, 'Name', 'EarlyWithdrawalTimeout', 'Timer', S.GUI.EarlyWithdrawalTimeout,...
        'StateChangeConditions', {'Tup', 'SampleStimTimerEW', Manual_Skip, 'SkipTrial'},...
        'OutputActions', {});
     ...
     sma = AddState(sma, 'Name', 'SkipTrial', 'Timer', 0,...
...
Skip_trials = zeros(1,Data.nTrials);
for x = 1:Data.nTrials
    ...
    if isnan(Data.RawEvents.Trial{x}.States.SkipTrial(1))
        Skip_trials(x) = 0;
    elseif ~isnan(Data.RawEvents.Trial{x}.States.SkipTrial(1))
        Skip_trials(x) = 1;
    end
end
BpodSystem.Data.SessionPerformance = Outcomes;
BpodSystem.Data.Skip_trials = Skip_trials;
SaveBpodSessionData;
