function NMTP_Outer_Testing_Nested
...
%     S.GUI.DelayHoldTime = 2.3;    
    ...
    S.GUI.PunishTime = 15;
    S.GUI.ITI = 10; 
    S.GUI.EarlyWithdrawalTimeout = 25;
    S.GUI.ProtocolTime = 60;
end
...
for fill = 1:12
    block = randperm(20);
    TrialTypes(fill*20-19:fill*20) = block;
end
for fill = 1:7
    block = randperm(4) + 16;
...
theta_freq = 10;
gamma_freq = 40;
sine_10hz = 5*(1+sin(2*pi*theta_freq*t-pi/2));
sine_10hz = sine_10hz(1:1000);
sine_40hz = 2.5*(1+sin(2*pi*gamma_freq*t-pi/2));
sine_40hz_offset = cat(2, sine_40hz(126:1000), sine_40hz(1:125));
sine_40hz = sine_40hz(1:1000);
thetaNestedGammaPeaks = (2/3)*(sine_10hz + sine_40hz);
thetaNestedGammaTroughs = (2/3)*(sine_10hz + sine_40hz_offset);
thetaNestedGammaTroughs = thetaNestedGammaTroughs - min(thetaNestedGammaTroughs);

freqs = {'p', 't', 'p', 't', 'n'};
% elements = {"Sample", "Sample", "Delay", "Delay", "Off"};
sampleTriggers = {{'BNC1', 1}, {'BNC1', 1}, {}, {}, {}};
delayTriggers = {{}, {}, {'BNC1', 1}, {'BNC1', 1}, {}};

ProgramPulsePalParam(1, 10, S.GUI.ProtocolTime);
ProgramPulsePalParam(1, 11 , 0);
ProgramPulsePalParam(1, 12, 1);
ProgramPulsePalParam(1, 14, 1);
ProgramPulsePalParam(1, 16 , 1);
ProgramPulsePalParam(1, 128, 2);
ProgramPulsePalParam(2, 10, S.GUI.ProtocolTime);
ProgramPulsePalParam(2, 11 , 0);
ProgramPulsePalParam(2, 12, 1);
ProgramPulsePalParam(2, 14, 1);
ProgramPulsePalParam(2, 16 , 1);
ProgramPulsePalParam(2, 128, 2);
...
for currentTrial = 1:MaxTrials
    ...
    %Create waveform
    stimType = ceil(TrialTypes(currentTrial)/4);
    switch freqs{stimType}
    case 'p' 
        SendCustomWaveform(1, 0.0001, thetaNestedGammaPeaks);
        SendCustomWaveform(2, 0.0001, thetaNestedGammaPeaks);
    case 't'
        SendCustomWaveform(1, 0.0001, thetaNestedGammaTroughs);
        SendCustomWaveform(2, 0.0001, thetaNestedGammaTroughs);
      
    end
    sampleTrigger = sampleTriggers{stimType};
    delayTrigger = delayTriggers{stimType};
    
    mod(TrialTypes(currentTrial),4)
    switch mod(TrialTypes(currentTrial),4)
        case 1
...
%             ChoiceValve = {'Valve5', 1}; ChoiceValveTime = GetValveTimes(S.GUI.ChoiceReward, 5);
    ...
    sma = AddState(sma, 'Name', 'WaitForSamplePoke', 'Timer', 0,...
        ...
        WrongPortsInSample(4), 'SampleOnHoldPunish'], 'OutputActions',...
        [sampleTrigger, SampleLight, 'Wire1', 1]);
    ...
    sma = AddState(sma, 'Name', 'WaitForSamplePokeEW', 'Timer', 0,...
        ...
        WrongPortsInSample(4), 'SampleOnHoldPunishEW'], 'OutputActions', [sampleTrigger, SampleLight, 'Wire1', 1]);
...
%         'OutputActions', {'Wire2', 1});
    ...
    sma = AddState(sma, 'Name', 'DelayTimer', 'Timer', 0,...
        ...
        'OutputActions', ['GlobalTimerTrig', 1, 'PWM7', 50, delayTrigger]);
    ...
    sma = AddState(sma, 'Name', 'DelayOnHold', 'Timer', S.GUI.DelayHoldTime,...
        ...
        'OutputActions', ['PWM7', 50, delayTrigger]);
    ...
    sma = AddState(sma, 'Name', 'EarlyWithdrawal', 'Timer', 3,...
        ...
        'OutputActions', [delayTrigger, 'Valve6', 1]);
